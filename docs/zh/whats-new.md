...
- **装配移进构造函数，`init()` 退场。** `Canary(Root)` 一返回，图已经建好、排好序、
  依赖已注入 —— `canary[SomeUnit]` 立刻可用。这是在守框架自己给单元定的规矩（"构造完就
  必须可用"）：运行时没有理由例外。

  切口从此划在**性质变化**的地方：装配是同步的、确定性的、不跑使用者的运行期代码；
  `start()` 才是运行（先全部 `@on_init`，再全部 `@on_start`）。装配期的错误现在在你写下
  `Canary(Root)` 那一行抛出。

  失败规则也因此从两条变成一条：**`stop()` 收台账里的一切**。状态机从 8 个降到 6 个，
  起点从 `NEW` 改名 `READY`。
