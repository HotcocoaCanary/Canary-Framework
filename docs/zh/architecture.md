...
建图用显式栈而不是递归 —— 依赖链的深度是使用者的数据，不该受 Python 递归上限约束。构造
也是先调用、出了 `TypeError` 再回头看签名：取签名只为了在失败时说清楚话，不该摊到每一个
成功构造的单元上（那曾占建图九成的时间）。
...
OpenAPI，约 1300 行），但它和 FastAPI / Starlette 做的是同一件事，而那不是这个框架的差异
化所在 —— 差异化在依赖装配与生命周期。重复造一遍只会让我们被迫在 WebSocket、文件上传、
中间件这些方向上永远追赶，而且每一个 bug 都出在那半边。
...
3. **框架只造空壳。** 一切需要外界输入的事都在生命周期里做 —— 因为只有生命周期里的事情
   才有对应的回收步骤。
4. **生命周期是显式的。** 装配在 `Canary(...)` 里，运行由你或宿主的 lifespan 驱动。
5. **静默失效必须响。** 写了、没报错、也没生效是最难查的问题，所以装配期宁可拒绝。
6. **能力只在使用者自己搭不出来时才由框架提供。** 十行能自己写的东西不该占一个公开入口。
7. **不重复造别人已经做好的东西。** 站在现有 Python 生态之上，而不是和它竞争。
