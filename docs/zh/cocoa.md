...
**类型即身份。** 单元按类型索引，一个类型在一张图里只有一个实例。"两个连不同库的
`Database`"因此写不出来 —— 需要两个实例就写两个类。

**注入的属性静态类型检查器看不见。** `self.config` 是运行期才出现的，mypy / pyright 无法
推断它。若需要类型检查通过，可在类体里写一条注解（`config: Config`）作为声明。
